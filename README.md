...
└── server.py
```
Proyek ini mencakup berbagai file dan direktori, masing-masing dengan tujuan spesifiknya sendiri: Folder `app`, merupakan folder aplikasi Flask utama. Folder `ml`, berisi model pembelajaran mesin untuk analisis sentimen dan klasifikasi, termasuk model IndoBERT dan model Support Vector Machine (SVM) untuk analisis sentimen dan deteksi niat. Folder `model`, berisi kelas-kelas yang mewakili struktur database. Folder `res`, menampung berkas csv hasil impor. Folder `routes`, berisi file rute untuk aplikasi Flask. Folder `static`, berisi aset statis seperti gambar, stylesheet CSS dan file JavaScript. Folder `templates`, berisi file template HTML untuk antarmuka Aplikasi Flask. Folder `utils`, berisi fungsi utilitas untuk prapemrosesan dan analisis data. Folder `helpers`, berisi beragam fungsi untuk utilitas aplikasi.
